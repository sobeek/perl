package DBI::ActiveRecord::DB::AnyDB;
use Mouse;
...
C<DBI::ActiveRecord::DB::AnyDB> - базовый класс-адаптер для работы с любой БД.

=head1 DESCRIPTION

Данный класс реализует общие методы для работы с БД.
...
Данные для полей C<$fields> обновляются значениями C<$values> в таблице C<$table>. Обновляются данные по первичному ключу C<$key_field> со значением C<$key_value>.
...
sub _update {
    ...
    my $fields_str_with_placeholders = (join " = ?, ", @$fields)." = ?";

    $dbh->begin_work;
    if ($dbh->do ("UPDATE $table SET $fields_str_with_placeholders WHERE $key_field = $key_value", {}, @$values)) {
        $dbh->commit;
        return 1;
    }
    else {
