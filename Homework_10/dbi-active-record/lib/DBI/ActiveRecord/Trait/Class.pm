...
Имя поля, значение для которого определяется самой БД. Значение данного поля будет игнорироваться при запросах вставки. Может быть не задано. Если задано, то всегда совпдает с C<primary_key>.
...
Так же метод создает набор селекторов для выборки данных из БД. Для каждого поля C<$field>, которое определенно как индексное (параметр C<index> атрибута), создается метод вида:
